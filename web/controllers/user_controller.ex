defmodule CodeBlog.UserController do
  ...
  defp authorize_user(conn, _) do
    ...
    if user && (Integer.to_string(user.id) == conn.params["id"] || CodeBlog.RoleChecker.is_admin?(user)) do
      conn
    else
      conn
  ...
  defp authorize_admin(conn, _) do
    user = get_session(conn, :current_user)
    if user && CodeBlog.RoleChecker.is_admin?(user) do
      conn
    else
      conn
